---
description: Reviews Python changes in the keystone clearing-risk library. Use after any non-trivial edit to engine, pricers, risk measures, or reconciliation code. Focuses on accuracy correctness, type safety, and the architectural seams.
...
Apply the `python-patterns` instructions (Pythonic idioms, PEP 8, type hints) as
the baseline standard for every review.

Review the current changes (use the `changes` tool / `git diff`). Check, in order:
