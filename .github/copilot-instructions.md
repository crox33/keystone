# keystone — repository instructions for GitHub Copilot
...
These live in `.github/instructions/` and Copilot applies them automatically by
file glob:
- `keystone-architecture.instructions.md` — the conventions, for any code under
  `src/keystone/`, `scripts/`, `tests/`.
- `market-data-cleaning.instructions.md` — for ingestion/cleaning code.
- `python-patterns.instructions.md` — idiomatic Python, for any `*.py`.
- `python-testing.instructions.md` — pytest/TDD, for test files.

## Custom chat modes (delegate proactively)
Pick these from the Copilot Chat mode dropdown (defined in `.github/chatmodes/`):
...
- Scripts are one-click runnable/debuggable in VSCode & PyCharm (committed run configs); no terminal needed.
